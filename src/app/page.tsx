...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Configuração para ouvir o fim do vídeo
  ...
  const tocarProximo = () => {
    const indice = videos.findIndex(v => (v.url?.includes(videoSelecionado) || v.videoId === videoSelecionado));
    ...
    <div className="min-h-screen bg-[#0f0f0f] text-white font-sans p-8">
      ...
      {videoSelecionado && (
        <div className="fixed inset-0 bg-black flex flex-col items-center justify-center z-50">
          <button onClick={() => setVideoSelecionado(null)} className="absolute top-5 right-5 text-white text-5xl hover:text-red-600">×</button>
          <div className="w-full max-w-5xl aspect-video">
            <iframe 
              src={`https://www.youtube.com/embed/${videoSelecionado}?autoplay=1&enablejsapi=1&rel=0`}
              className="w-full h-full"
              allow="autoplay; encrypted-media"
              allowFullScreen
            />
          </div>
          <p className="mt-4 text-zinc-500 italic">Modo Maratona: O próximo vídeo começará sozinho!</p>
        </div>
      )}
    </div>
  );
